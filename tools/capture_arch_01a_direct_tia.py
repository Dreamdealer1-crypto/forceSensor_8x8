#!/usr/bin/env python3
"""Interactive raw capture for ORDER-ARCH-01A direct TIA validation."""
...
MEASURED_KEYS = [
    ("Rf_measured_ohm", "Rf feedback resistor actual value, ohm", None),
    ("R10_measured_ohm", "10k resistor actual value, ohm", 10_000.0),
    ("R22_measured_ohm", "22k resistor actual value, ohm", 22_000.0),
    ("R47_measured_ohm", "47k resistor actual value, ohm", 47_000.0),
    ("R68_measured_ohm", "68k resistor actual value, ohm", 68_000.0),
    ("R100_measured_ohm", "100k resistor actual value, ohm", 100_000.0),
    ("R220_measured_ohm", "220k resistor actual value, ohm", 220_000.0),
    ("R470_measured_ohm", "470k resistor actual value, ohm", 470_000.0),
    ("R1p5M_measured_ohm", "1.5M resistor actual value, ohm", 1_500_000.0),
    ("3V3_pre_v", "3V3 before sweep, V", None),
    ("VREF_pre_v", "VREF before sweep, V", None),
]
POST_KEYS = [
    ("3V3_post_v", "3V3 after sweep, V", None),
    ("VREF_post_v", "VREF after sweep, V", None),
]
...
def prompt_measurements(items: list[tuple[str, str, Optional[float]]]) -> dict[str, object]:
    ...
    for key, label, nominal in items:
        suffix = " [Enter=unavailable]" if nominal is None else f" [Enter=use nominal {nominal:g}]"
        while True:
...
def run_capture(args: argparse.Namespace) -> int:
    ...
    if args.dataset_dir:
        ...
        dataset_dir = ROOT / "data" / "arch_01a_direct_tia" / f"{timestamp}_{args.run}"
        dataset_dir.mkdir(parents=True, exist_ok=False)

    print("ORDER-ARCH-01A direct TIA capture")
    print("Hardware required: real sensor disconnected, 4051 disabled by firmware, Rtest directly U1 Pin2 -> GND.")
    print("Enter measured values. Do not invent unavailable values.")
    ...
    with serial.Serial(args.port, args.baud, timeout=args.timeout) as serial_port:
        ...
        for condition, _nominal in CONDITIONS[start_index:]:
            while True:
                input(f"\nConnect {condition}, wait until stable, then press ENTER.")
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Capture ORDER-ARCH-01A direct TIA raw data.")
